// ============================================================
// 求职小助 v3.3 Cloudflare Worker
// 架构：画像池中心 + 四层输入 + 四类AI输出 + 面经墙UGC
// 生成时间: 2026-05-04
// ============================================================
...
async function runMigration(env) {
  ...
  try {
    ...
    // --- v3.3 面经墙 UGC 生态 ---
    if (!(await hasTable('experience_cards'))) {
      await env.DB.prepare(`
        CREATE TABLE experience_cards (
          id TEXT PRIMARY KEY, user_id TEXT NOT NULL, review_id TEXT,
          company TEXT NOT NULL, position_title TEXT NOT NULL,
          round_number INTEGER, round_type TEXT, score INTEGER,
          card_mode TEXT NOT NULL DEFAULT 'qa', card_title TEXT,
          card_body TEXT NOT NULL, ai_tags TEXT,
          like_count INTEGER DEFAULT 0, question_count INTEGER DEFAULT 0,
          is_draft INTEGER DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL
        )
      `).run();
      log.push('✅ experience_cards 表已创建');
    } else log.push('⏭️  experience_cards 表已存在');

    if (!(await hasTable('card_questions'))) {
      await env.DB.prepare(`
        CREATE TABLE card_questions (
          id TEXT PRIMARY KEY, card_id TEXT NOT NULL, asker_id TEXT NOT NULL,
          question_text TEXT NOT NULL, is_ai_generated INTEGER DEFAULT 0,
          reply_text TEXT, replied_at TEXT, like_count INTEGER DEFAULT 0,
          is_hidden INTEGER DEFAULT 0, created_at TEXT NOT NULL
        )
      `).run();
      log.push('✅ card_questions 表已创建');
    } else log.push('⏭️  card_questions 表已存在');

    if (!(await hasTable('card_likes'))) {
      await env.DB.prepare(`
      ...
      `).run();
      await env.DB.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_likes_unique ON card_likes(user_id, target_type, target_id)').run();
      log.push('✅ card_likes 表已创建');
    } else log.push('⏭️  card_likes 表已存在');

    if (!(await hasTable('card_ai_questions'))) {
      await env.DB.prepare(`
        CREATE TABLE card_ai_questions (
          card_id TEXT PRIMARY KEY, questions TEXT NOT NULL, generated_at TEXT NOT NULL
        )
      `).run();
      log.push('✅ card_ai_questions 表已创建');
    } else log.push('⏭️  card_ai_questions 表已存在');

    // 用户 profile 扩展
    if (!(await hasColumn('users', 'display_name'))) {
      await env.DB.prepare('ALTER TABLE users ADD COLUMN display_name TEXT').run();
      log.push('✅ users.display_name 已添加');
    } else log.push('⏭️  users.display_name 已存在');

    if (!(await hasColumn('users', 'bio'))) {
      await env.DB.prepare('ALTER TABLE users ADD COLUMN bio TEXT').run();
      log.push('✅ users.bio 已添加');
    } else log.push('⏭️  users.bio 已存在');
...
async function handleListCards(request, env) {
  ...
  return jsonResp({ cards: results || [] });
}

// GET /api/cards/:id
async function handleGetCard(request, env, id) {
  const { user } = await requireAuth(request, env);
  if (user.error) return user.error;
  const uid = user.user ? user.user.id : user.id;

  const card = await env.DB.prepare(
    `SELECT c.*, u.name AS author_name, u.display_name AS author_display, u.bio AS author_bio
     FROM experience_cards c LEFT JOIN users u ON c.user_id = u.id WHERE c.id = ?`
  ).bind(id).first();
  if (!card) return errResp('not found', 404);

  const liked = await env.DB.prepare(
    "SELECT id FROM card_likes WHERE user_id = ? AND target_type = 'card' AND target_id = ?"
  ).bind(uid, id).first();

  return jsonResp({ card: { ...card, is_liked: !!liked } });
}
...
async function handleCreateCard(request, env) {
  ...
  const uid = user.user ? user.user.id : user.id;
  const body = await readJson(request) || {};
  if (!body.card_body || body.card_body.trim().length < 20) return errResp('内容太短');

  const id = genId('ec');
  const ts = now();
  const isDraft = body.publish ? 0 : 1;

  await env.DB.prepare(`
    INSERT INTO experience_cards (id, user_id, review_id, company, position_title,
      round_number, round_type, score, card_mode, card_title, card_body, ai_tags,
      is_draft, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    id, uid, body.review_id || null, body.company || '', body.position_title || '',
    body.round_number || null, body.round_type || null, body.score || null,
    body.card_mode || 'qa', body.card_title || null, body.card_body.trim(),
    body.ai_tags ? JSON.stringify(body.ai_tags) : null,
    isDraft, ts, ts
  ).run();

  return jsonResp({ ok: true, id, is_draft: isDraft });
}
...
async function handleUpdateCard(request, env, id) {
  ...
  const card = await env.DB.prepare('SELECT * FROM experience_cards WHERE id = ? AND user_id = ?')
    .bind(id, uid).first();
  if (!card) return errResp('not found', 404);

  const updates = []; const bindings = [];
  if (body.card_body) { updates.push('card_body = ?'); bindings.push(body.card_body.trim()); }
  if (body.card_title !== undefined) { updates.push('card_title = ?'); bindings.push(body.card_title); }
  if (body.is_draft !== undefined) { updates.push('is_draft = ?'); bindings.push(body.is_draft ? 1 : 0); }
  if (body.ai_tags) { updates.push('ai_tags = ?'); bindings.push(JSON.stringify(body.ai_tags)); }
  if (updates.length === 0) return jsonResp({ ok: true });

  updates.push('updated_at = ?'); bindings.push(now()); bindings.push(id);
  await env.DB.prepare(`UPDATE experience_cards SET ${updates.join(', ')} WHERE id = ?`)
    .bind(...bindings).run();
  return jsonResp({ ok: true });
}
...
async function handleDeleteCard(request, env, id) {
  ...
  await env.DB.prepare('DELETE FROM card_questions WHERE card_id = ?').bind(id).run();
  await env.DB.prepare("DELETE FROM card_likes WHERE target_type = 'card' AND target_id = ?").bind(id).run();
  await env.DB.prepare('DELETE FROM card_ai_questions WHERE card_id = ?').bind(id).run();
  await env.DB.prepare('DELETE FROM experience_cards WHERE id = ? AND user_id = ?').bind(id, uid).run();

  return jsonResp({ ok: true });
}

// POST /api/cards/:id/questions/generate
async function handleGenerateCardQuestions(request, env, id) {
  const card = await env.DB.prepare('SELECT * FROM experience_cards WHERE id = ?').bind(id).first();
  if (!card) return errResp('not found', 404);

  const cached = await env.DB.prepare('SELECT * FROM card_ai_questions WHERE card_id = ?').bind(id).first();
  if (cached) return jsonResp({ questions: JSON.parse(cached.questions) });

  const prompt = `根据以下面经卡信息，生成 6 条自然、有价值的追问。\n公司：${card.company || '未知'}\n岗位：${card.position_title || '未知'}\n轮次：${card.round_number ? '第' + card.round_number + '轮' : '未知'}${card.round_type ? ' · ' + card.round_type : ''}\n评分：${card.score ? card.score + '/10' : '未知'}\n内容：${card.card_body.slice(0, 800)}\n\n要求：每条 8-25 字，口语化；涵盖准备建议、面试官风格、技术难度、踩坑经验；禁止薪资问题。只输出 JSON：{"questions":["..."]}`;

  try {
    const uid = card.user_id;
    const { content } = await runLLMWithQuota(env, { id: uid }, 'FAST', {
      messages: [{ role: 'user', content: prompt }],
      temperature: 0.7, json: true, purpose: 'card_questions',
    });
    const parsed = JSON.parse(content);
    const questions = (parsed.questions || []).slice(0, 8);

    if (questions.length > 0) {
      await env.DB.prepare('INSERT OR REPLACE INTO card_ai_questions (card_id, questions, generated_at) VALUES (?, ?, ?)')
        .bind(id, JSON.stringify(questions), now()).run();
    }
    return jsonResp({ questions });
  } catch (e) {
    const fallback = ['面试官是什么风格？', '一共面了几轮？', '这个岗位最看重什么能力？', '你准备了多久？', '有什么踩坑的地方？', '推荐什么准备方式？'];
    return jsonResp({ questions: fallback });
  }
}

// GET /api/cards/:id/questions
async function handleGetCardQuestions(request, env, id) {
  const { results } = await env.DB.prepare(
    `SELECT q.*, u.name AS asker_name, u.display_name AS asker_display
     FROM card_questions q LEFT JOIN users u ON q.asker_id = u.id
     WHERE q.card_id = ? ORDER BY q.like_count DESC, q.created_at ASC`
  ).bind(id).all();
  return jsonResp({ questions: results || [] });
}

// POST /api/cards/:id/questions
async function handleAskQuestion(request, env, id) {
  const { user } = await requireAuth(request, env);
  if (user.error) return user.error;
  const uid = user.user ? user.user.id : user.id;
  const body = await readJson(request) || {};
  if (!body.question_text || body.question_text.trim().length < 5) return errResp('问题太短');

  const card = await env.DB.prepare('SELECT * FROM experience_cards WHERE id = ?').bind(id).first();
  if (!card) return errResp('not found', 404);

  const qid = genId('cq');
  const isAiGen = body.is_ai_generated ? 1 : 0;
  const isHidden = isAiGen ? 0 : 1; // 自定义问题默认折叠

  await env.DB.prepare(`
    INSERT INTO card_questions (id, card_id, asker_id, question_text, is_ai_generated, is_hidden, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(qid, id, uid, body.question_text.trim(), isAiGen, isHidden, now()).run();

  await env.DB.prepare('UPDATE experience_cards SET question_count = question_count + 1, updated_at = ? WHERE id = ?')
    .bind(now(), id).run();

  if (card.user_id !== uid) {
    await sendCardNotif(env, card.user_id, 'question_asked', '有人向你提问了',
      body.question_text.trim().slice(0, 80), id);
  }

  return jsonResp({ ok: true, id: qid });
}

// PUT /api/cards/:id/questions/:qid
async function handleReplyQuestion(request, env, id, qid) {
  const { user } = await requireAuth(request, env);
  if (user.error) return user.error;
  const uid = user.user ? user.user.id : user.id;
  const body = await readJson(request) || {};
  if (!body.reply_text || body.reply_text.trim().length < 2) return errResp('回复太短');

  const card = await env.DB.prepare('SELECT * FROM experience_cards WHERE id = ?').bind(id).first();
  if (!card) return errResp('not found', 404);
  if (card.user_id !== uid) return errResp('仅卡主可回复', 403);

  const q = await env.DB.prepare('SELECT * FROM card_questions WHERE id = ? AND card_id = ?')
    .bind(qid, id).first();
  if (!q) return errResp('not found', 404);

  await env.DB.prepare(
    'UPDATE card_questions SET reply_text = ?, replied_at = ?, is_hidden = 0 WHERE id = ?'
  ).bind(body.reply_text.trim(), now(), qid).run();

  if (q.asker_id !== uid) {
    await sendCardNotif(env, q.asker_id, 'question_replied', '你的问题有了回复',
      body.reply_text.trim().slice(0, 80), id);
  }
...
async function handleLike(request, env) {
  ...
  const uid = user.user ? user.user.id : user.id;
  const body = await readJson(request) || {};
  if (!body.target_type || !body.target_id) return errResp('缺少参数');

  const existing = await env.DB.prepare(
    'SELECT id FROM card_likes WHERE user_id = ? AND target_type = ? AND target_id = ?'
  ).bind(uid, body.target_type, body.target_id).first();
...
async function handleUpdateProfile(request, env) {
  ...
  const updates = []; const bindings = [];
  if (body.display_name !== undefined) { updates.push('display_name = ?'); bindings.push(body.display_name || null); }
  if (body.name !== undefined) { updates.push('name = ?'); bindings.push(body.name || null); }
  if (body.target_track !== undefined) { updates.push('target_track = ?'); bindings.push(body.target_track || null); }
  if (body.bio !== undefined) { updates.push('bio = ?'); bindings.push(body.bio || null); }
...
async function sendCardNotif(env, userId, type, title, content, refId) {
  try {
    const pref = await env.DB.prepare('SELECT enabled FROM notification_preferences WHERE user_id = ? AND type = ?')
      .bind(userId, type).first();
    if (pref && pref.enabled === 0) return;

    await env.DB.prepare(`
      INSERT INTO notifications (id, user_id, type, title, content, ref_id, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(genId('n'), userId, type, title, content || '', refId || '', now()).run();
  } catch (e) { /* 非关键路径 */ }
}

// ==================== 匹配打分（同 v3.2） ====================// ==================== 匹配打分：粗分（Flash）====================
...
async function handleRequest(request, env, ctx) {
  ...
  if (path === '/api/likes' && method === 'POST') return handleLike(request, env);
  ...
  if (mCard) {
    const cid = mCard[1];
    if (method === 'GET') return handleGetCard(request, env, cid);
    if (method === 'PUT') return handleUpdateCard(request, env, cid);
    if (method === 'DELETE') return handleDeleteCard(request, env, cid);
  }

  const mCardGen = path.match(/^\/api\/cards\/([\w-]+)\/questions\/generate$/);
  if (mCardGen && method === 'POST') return handleGenerateCardQuestions(request, env, mCardGen[1]);

  const mCardQs = path.match(/^\/api\/cards\/([\w-]+)\/questions$/);
  if (mCardQs) {
    const cid = mCardQs[1];
    if (method === 'GET') return handleGetCardQuestions(request, env, cid);
    if (method === 'POST') return handleAskQuestion(request, env, cid);
  }

  const mCardQReply = path.match(/^\/api\/cards\/([\w-]+)\/questions\/([\w-]+)$/);
  if (mCardQReply && method === 'PUT') return handleReplyQuestion(request, env, mCardQReply[1], mCardQReply[2]);
